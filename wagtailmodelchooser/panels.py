...
class ModelChooserPanel(FieldPanel):
    ...
    def get_admin_urls_for_registration(cls):
        return (
            re_path(
                r"^{}/chooser/$".format(cls.model._meta.model_name),
                cls.chooser_view.as_view(),
                name="{}_chooser".format(cls.model._meta.model_name),
            ),
            re_path(
                r"^{}/chooser/(?P<pk>\d+)/$".format(cls.model._meta.model_name),
                cls.chosen_view.as_view(),
...
def register_chooser_for_model(
    ...
    assert issubclass(
        model, models.Model
    ), "model must be a subclass of Django's models.Model"
    ...
    if widget_class:
        if not getattr(widget_class, "model", None) == model:
            raise AssertionError(
                "Incorrect model attribute for {}: '{}.{}' is expected".format(
        ...
        widget_class = type(
            str("Admin{}Chooser".format(class_name)), (AdminModelChooser,), defaults
        )
    ...
    if chooser_view:
        ...
        assert hasattr(
            chooser_view, "title_field"
        ), "chooser_view must define 'title_field'"
        assert hasattr(
            chooser_view, "model_chooser_url_name"
        ), "chooser_view must define 'model_chooser_url_name'"
        assert hasattr(
            chooser_view, "model_chosen_url_name"
        ), "chooser_view must define 'model_chosen_url_name'"
    else:
        defaults = {
            "model": model,
            "ordering": ["-pk"],
        ...
        chooser_view = type(
            str("{}ChooserView".format(class_name)), (ModelChooserView,), defaults
        )
    ...
    if chosen_view:
        ...
        assert hasattr(
            chosen_view, "title_field"
        ), "chooser_view must define 'title_field'"
    else:
        defaults = {"model": model, "title_field": title_field}
        defaults.update(**chosen_view_attrs)
        chosen_view = type(
            str("{}ChosenView".format(class_name)), (ModelChosenView,), defaults
        )

    # Build the chooser panel class
    chooser_panel_class = type(
        str("{}ChooserPanel".format(class_name)),
        (ModelChooserPanel,),
        {
