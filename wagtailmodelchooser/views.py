...
class ModelChooserView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context.update(
            model_chooser_url_name=self.model_chooser_url_name,
    ...
    def get(self, request, *args, **kwargs):
        ...
        if "q" in self.request.GET or "p" in self.request.GET:
            ...
            self.object_list = paginator.get_page(request.GET.get("p"))
            context = self.get_context_data(
                searchform=searchform, query_string=searchform.cleaned_data.get("q")
            )
        ...
        return render_modal_workflow(
            request, self.template_name, self.js_handler_template_name, context
        )
...
class ModelChosenView(DetailView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        return render_modal_workflow(
            request,
            None,
            "wagtailmodelchooser/chooser/model_chosen.js",
            {
                "model_json": json.dumps(
                    self.model_data_extractor(self.object, self.title_field)
                )
            },
        )
